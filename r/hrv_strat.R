...
h_strat <- data.frame(mmb = c(0.2, 0.2, 0.5, 1.0, 1.5,
                              0.5, 0.5, 1.0, 1.5,
                              0.5, 1.0, 1.5),
                      harv_rate = c(0, 0.05, 0.05, 0.1, 0.1,
                                    0, 0.05, 0.1, 0.1,
                                    0, 0.1, 0.1),
                      scenario = c("HCR 1", "HCR 1", "HCR 1", 
                                   "HCR 1", "HCR 1",
                                   "HCR 2", "HCR 2", "HCR 2",
                                   "HCR 2",
                                   "HCR 3", "HCR 3", "HCR 3")
...
h_strat %>%
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),
                     ...
                     limits = c(0.0, 0.15),
                     expand = c(0,0)) +
  xlab(bquote(MMA/MMA[AVG])) +
  ylab("Exploitation rate on MMA") +
...
#Retrospective TAC recommendations via HCR 2 St. Matthew BKC 5 AAC 34.917
avg_mmb
avg_mmb_fifty

regional_biomass %>%
  mutate(mmb_non_survey = (adj_mature/0.528)-adj_mature,
...
rkc_tac_summary %>%
  select(year, harv_rate_mature, max_tac, harv_rate_lmb, max_tac_lmb) -> rkc_tac_summary_table

write.csv(rkc_tac_summary_table, paste0('./output/', cur_yr,'/rkc_tac_summary_table.csv'))


#Visualize MMB vs LMB scenarios----
rkc_tac_summary %>%
  ggplot(aes(year)) +
